...
def add_sparse_args(parser):
    ...
    parser.add_argument('--sparse_init', type=str, default='ERK', help='pruning mode for pruning and finetuning')
            ...
            if 'uniform' in self.pruning_mode:
                if 'G' in self.pruning_mode:
                    for name, tensor in self.G_model.named_parameters():
                        if name not in self.G_masks: continue
                        k = int(self.pruning_rate * self.G_masks[name].numel())
                        x, idx = torch.sort(torch.abs(tensor.data.view(-1)))
                        self.G_masks[name].data.view(-1)[idx[:k]] = 0.0

                if 'D' in self.pruning_mode:
                    for name, tensor in self.D_model.named_parameters():
                        if name not in self.D_masks: continue
                        k = int(self.pruning_rate * self.D_masks[name].numel())
                        x, idx = torch.sort(torch.abs(tensor.data.view(-1)))
                        self.D_masks[name].data.view(-1)[idx[:k]] = 0.0


            elif 'global' in self.pruning_mode:
                if 'G' in self.pruning_mode:
                    ...
                    for name, weight in self.G_model.named_parameters():
                        if name not in self.G_masks: continue
                        self.G_masks[name] = ((torch.abs(weight)) >= acceptable_score).float()
                ...
                if 'D' in self.pruning_mode:
                    ...
                    for name, weight in self.D_model.named_parameters():
                        if name not in self.D_masks: continue
                        self.D_masks[name] = ((torch.abs(weight)) >= acceptable_score).float()
