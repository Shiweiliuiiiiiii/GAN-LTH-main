...
def add_sparse_args(parser):
    ...
    parser.add_argument('--pruning_rate', type=float, default=0.06, help='pruning rate for pruning and finetuning.')
...
class Masking(object):
    def __init__(self, optimizer_G=False, optimizer_D_A=False, optimizer_D_B=False, death_rate_decay=False, death_rate=0.3, death='magnitude', G_growth='gradient', D_growth='random', redistribution='momentum', args=None):
        ...
        self.netD_A = None
        self.netD_B = None
        self.pruning_mode = args.pruning_mode
    ...
    def init(self, mode='ERK', density=0.05, densityG=0.5, erk_power_scale=1.0):
        ...
        if mode == 'ERK':
            ...
            print('pruning...')
            if 'uniform' in self.pruning_mode:
                if 'G' in self.pruning_mode:
                    print('pruning G...')
                    self.uniform_pruning(self.netG_A2B, self.netG_A2B_masks)
                    self.uniform_pruning(self.netG_B2A, self.netG_B2A_masks)

                if 'D' in self.pruning_mode:
                    print('pruning D...')
                    self.uniform_pruning(self.netD_A, self.netD_A_masks)
                    self.uniform_pruning(self.netD_B, self.netD_B_masks)

            if 'global' in self.pruning_mode:
                if 'G' in self.pruning_mode:
                    print('pruning G...')
                    self.global_pruning(self.netG_A2B, self.netG_A2B_masks)
                    self.global_pruning(self.netG_B2A, self.netG_B2A_masks)

                if 'D' in self.pruning_mode:
                    print('pruning D...')
                    self.global_pruning(self.netD_A, self.netD_A_masks)
                    self.global_pruning(self.netD_B, self.netD_B_masks)
    ...
    def add_module(self, netG_A2B, netG_B2A, netD_A, netD_B, densityG=0.5 , density=0.5, sparse_init='ERK'):
        ...
        for name, tensor in self.netG_A2B.named_parameters():
            if len(tensor.size()) == 4:
                self.netG_A2B_masks[name] = torch.ones_like(tensor,  requires_grad=False).cuda()
        # add masks for netG_B2A
        for name, tensor in self.netG_B2A.named_parameters():
            if len(tensor.size()) == 4:
                self.netG_B2A_masks[name] = torch.ones_like(tensor,  requires_grad=False).cuda()
        # add masks for netD_A
        for name, tensor in self.netD_A.named_parameters():
            if len(tensor.size()) == 4:
                self.netD_A_masks[name] = torch.ones_like(tensor,  requires_grad=False).cuda()
        # add masks for netD_B
        for name, tensor in self.netD_B.named_parameters():
            if len(tensor.size()) == 4:
                self.netD_B_masks[name] = torch.ones_like(tensor,  requires_grad=False).cuda()
    ...
    def uniform_pruning(self, net, masks):
        for name, tensor in net.named_parameters():
            if name not in masks: continue
            k = int(self.pruning_rate * masks[name].numel())
            x, idx = torch.sort(torch.abs(tensor.data.view(-1)))
            masks[name].data.view(-1)[idx[:k]] = 0.0

    def global_pruning(self, net, masks):
        total_num_nonzoros = 0
        weight_abs = []
        for name, weight in net.named_parameters():
            if name not in masks: continue
            total_num_nonzoros += (weight != 0).sum().item()
            weight_abs.append(torch.abs(weight))

        # Gather all scores in a single vector and normalise
        all_scores = torch.cat([torch.flatten(x) for x in weight_abs])
        num_params_to_keep = int(total_num_nonzoros * (1 - self.pruning_rate))
