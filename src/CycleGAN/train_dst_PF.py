...
from sparselearning.core_cyclegan_PF import Masking, CosineDecay, LinearDecay
...
sparselearning.core_cyclegan_PF.add_sparse_args(parser)
...
experiment_name = experiment_name + '_%s' % (args.pruning_mode) + '_%f' % (args.pruning_rate)
...
netG_A2B.load_state_dict(torch.load(os.path.join(args.rand, 'netG_A2B_epoch_0.pth')))
netG_B2A.load_state_dict(torch.load(os.path.join(args.rand, 'netG_B2A_epoch_0.pth')))
netD_A.load_state_dict(torch.load(os.path.join(args.rand, 'netD_A_epoch_0.pth')))
netD_B.load_state_dict(torch.load(os.path.join(args.rand, 'netD_B_epoch_0.pth')))
