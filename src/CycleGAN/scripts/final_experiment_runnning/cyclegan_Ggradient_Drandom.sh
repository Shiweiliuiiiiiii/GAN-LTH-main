...
#SBATCH -J cyclegan_Ggradient_Drandom_0.5D           # 所运行的任务名称 (自己取)
...
#SBATCH -o cyclegan_Ggradient_Drandom_0.5D.out       # 打印输出的文件
...
do
  ...
  do
    ...
    --exp-name sparse_training_unbalanced_Ggradient_Drandom_densityG$density_fre1000_seed$seed \
    --dy_mode GD --densityG $density --G_growth gradient --D_growth random
  done
done
#conda activate GAN1
