...
#SBATCH -J Cyclegan_onlyD_0.5D           # 所运行的任务名称 (自己取)
...
#SBATCH -o Cyclegan_onlyD_0.5D.out       # 打印输出的文件
...
do
  ...
  do
    ...
    --exp-name sparse_training_unbalanced_onlyD_densityG$density_fre500_seed$seed \
    --dy_mode D --densityG $density --G_growth gradient --D_growth gradient
  done
done
#conda activate GAN1
