...
#SBATCH -J sngan_DST_Ggradient_Dgradient_fre          # 所运行的任务名称 (自己取)
...
#SBATCH -o sngan_DST_Ggradient_Dgradient_fre       # 打印输出的文件
...
#--------- unbalanced gan training (DST Ggradient Drandom)-----------------
 ...
 do
   python train.py --model sngan_cifar10 --exp-name sngan_cifar10_unbalanced_DST_Ggradient_Dgradient_fre$fre --init-path initial_weights \
   --sparse --imbalanced --sparse_init ERK --density 0.3 --densityG 0.05 --update_frequency $fre \
   --dy_mode GD --SEMA --G_growth gradient --D_growth gradient --random_seed 1
 done
