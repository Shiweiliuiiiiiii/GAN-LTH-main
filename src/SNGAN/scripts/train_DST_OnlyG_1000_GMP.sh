...
do
 ...
 do
   python train.py --model sngan_cifar10 --exp-name sngan_cifar10_unbalanced_GMP_onlyG_fre5000_seed$s --init-path initial_weights \
   --sparse --imbalanced --sparse_init ERK --densityD $densityD --densityG $densityG --update_frequency 100 \
   --dy_mode G --SEMA --G_growth gradient --D_growth random --random_seed $s --sparse_mode GMP --initial_prune_time 0.0
 done
done
