...
#SBATCH -J SNGAN_unbalanced_sparse_training_static_variousD          # 所运行的任务名称 (自己取)
...
#SBATCH -o SNGAN_unbalanced_sparse_training_static_variousD.out       # 打印输出的文件
...
do
  ...
  do
  python train.py --SEMA --model sngan_cifar10 --exp-name sngan_cifar10_unbalanced_seed1 --init-path initial_weights \
  --sparse --imbalanced --sparse_init ERK --densityD $densityD --densityG $densityG --update_frequency 4000 --random_seed 1
  done
done
