...
#SBATCH -J sngan_DST_onlyD_densityD0.5         # 所运行的任务名称 (自己取)
...
#SBATCH -o sngan_DST_onlyD_densityD0.5.out       # 打印输出的文件
...
do
 ...
 do
   python train.py --model sngan_cifar10 --exp-name sngan_cifar10_unbalanced_DST_onlyD_fre1000_seed$s --init-path initial_weights \
   --sparse --imbalanced --sparse_init ERK --densityD $densityD --densityG $densityG --update_frequency 1000 \
   --dy_mode D --SEMA --G_growth gradient --D_growth random --random_seed $s
 done
done
